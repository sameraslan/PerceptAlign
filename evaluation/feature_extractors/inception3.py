...
class FeatureExtractorInceptionV3(TorchVisionInception3):
    ...
    def __init__(self, name, features_list, feature_extractor_weights_path=None, **kwargs):
        ...
        assert type(features_list) in (list, tuple, ListConfig), 'Wrong features list type'
        assert all((a in self.provided_feats for a in features_list)), 'Requested features arent on the list'
    ...
    def forward(self, x):
        assert torch.is_tensor(x) and x.dtype == torch.uint8, 'Expecting x as torch.Tensor, dtype=torch.uint8'
        features = {}
