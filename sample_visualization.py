...
def rename_models(x):
    # st.write(f"Value of x: {x}")  # This will display the value in the Streamlit app
    x = x[x.index('T')+1:] if 'T' in x else x
    name2type = {
...
if __name__ == "__main__":
    ...
    This is a demo for **Perceptual Alignment** 🚀🚀🚀.
    ...
    if opt.resume:
        ...
        if os.path.isfile(opt.resume):
            ...
            if 'best.ckpt' in ckpt_files:
                ckpt_file = 'best.ckpt'
                st.info(f'Using "best.ckpt" as the checkpoint file from model "{logdir}".')
            else:
                # Use the first checkpoint from the sorted list
                ckpt_file = sorted(ckpt_files)[0]
                if len(ckpt_files) > 1:
                    st.warning(f'Warning: "best.ckpt" not found. Using {ckpt_file} instead.')
                else:
                    st.info(f'Using {ckpt_file} as the checkpoint file  from model {logdir}.')
