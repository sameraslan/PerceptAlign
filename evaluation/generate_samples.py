...
def load_and_save_config():
    ...
    dataset_name = dataset_name[:dataset_name.find("SpecsCondOnFeatsTrain")].lower() if "SpecsCondOnFeatsTrain" in dataset_name else dataset_name.lower()
        ...
        if config_cli.data is None:
            if cfg.data.params[a] is not None:
                if 'vggsound.VGGSound' in cfg.data.params.train.target:
                    ...
                    cfg.data.params[a] = os.path.join(f'./data/{dataset_name}/features/*', Path(cfg.data.params[a]).name)
    # save the config
    save_config(cfg)
    return cfg
...
def make_folder_for_samples(cfg):
    ...
    samples_split_dirs = {}
    print("CFG splits", cfg.sampler.splits)
...
# Functionality to resume sampling
def batch_specs_exist(cfg, samples_split_dirs, model, batch, split, sample_id, vocoder):
    if isinstance(model, torch.nn.parallel.DistributedDataParallel):
        # model_ddp = model
        model = model.module
    ...
    # iterating inside a batch
    for i in range(len(batch[specs_key])):
        class_foldername = f'cls_{batch["target"][i]}'
        vidname = extract_vidname_fn(batch[specs_key][i])
        folder_path = samples_split_dirs[split] / class_foldername
        video_path = folder_path / f'{vidname}_sample_{sample_id}.npy'
    ...
    for split, dataloader in dataloaders.items():
        for batch in tqdm(dataloader):
            for sample_id in range(samples_per_video):
                if not batch_specs_exist(cfg, samples_split_dirs, model, batch, split, sample_id, vocoder):
                    specs = sample_spectrogram(cfg, model, batch)
                    save_specs(cfg, specs, samples_split_dirs, model, batch, split, sample_id, vocoder)
                else:
                    print(f"Skipping generation for batch in {split}, sample {sample_id} as files already exist.")
