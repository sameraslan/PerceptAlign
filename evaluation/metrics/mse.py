...
def calculate_mse(mel_dict_1, mel_dict_2, dataset_name, classes=None):
    # Crop GT based on size of generation
    # Determine the minimum time dimension across both sets of spectrograms
    ...
    # Calculate MSE for each shared key
    for sharedkey, mel_2 in sharedkey_to_mel_2.items():
        ...
        # Calculate MSE for this set of generated samples
        mse = torch.mean((gen_mels - gt_mel) ** 2)
        mse_list.append(mse)

    # Calculate overall average MSE
    overall_mse = torch.mean(torch.stack(mse_list))

    return {
        'mean_squared_error': overall_mse.item()
    }
